...
const Navbar = () => {
  ...
  // Track if we're on tablet (between mobile and desktop)
  const [isTablet, setIsTablet] = useState(false);
  // Track if desktop hover is active
  const isHoveringRef = useRef(false);

  useEffect(() => {
    // Check if we're on mobile or tablet - use a throttled handler for better performance
  ...
  const handleProfileMouseEnter = (e) => {
    // Only apply hover behavior on desktop devices (not mobile or tablet)
  ...
  // For mobile and tablet devices, maintain click behavior
  const openModal = (e) => {
    // Only apply click behavior on mobile and tablet devices
    if (isMobile || isTablet) {
  ...
  return (
    <header className={`navbar ${scrolled ? "scrolled" : "not-scrolled"} ${navbarHidden ? "navbar-hidden" : ""}`}>
      <div className={`inner ${navbarHidden ? "inner-hidden" : ""}`}>
        ...
        >
          <div 
            className={`profile-pic ${showProfile && !isMobile && !isTablet ? 'hidden-on-desktop' : ''}`} 
      ...
      {/* Desktop profile modal (with tablet check) */}
      {showProfile && !isMobile && (
        <div 
          className={`profile-modal ${isClosing ? 'closing' : ''}`}
          onClick={closeModal}
        ...
        >
          ...
          >
            <div className="modal-header">
              <h3 className="modal-title">Shivesh Mishra</h3>
              {/* Add close button only for tablets */}
              {isTablet && (
                <button 
      ...
      <style jsx>{`
        ...
        /* Modal header styling for close button */
        .modal-header {
          display: flex;
